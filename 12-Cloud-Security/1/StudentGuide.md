...
- Virtual computing, creating VMs in the cloud instance, and using a jump box to access VNet.
...
Today's activities will take them through the basics of setting up a cloud environment protected by a firewall, and hosting a virtual machine that they will use as a jump box to access their cloud environment.
...
Some pros and cons of using a preconfigured service as opposed to building a custom solution:

- A preconfigured service is faster to set up.
- A preconfigured service requires less training and research for a system administrator.
...
Note that cloud providers all have the same abilities, but may look different. Additionally, different providers offer different levels of preconfigured services. Which provider an organization chooses will depend on their specific needs.
...
We'll only need to define the boundaries of the network. Azure does the rest. We will not need to configure a router or DHCP, as these are configured automatically.
...
**Important:** For this and all activities, make sure you are using their personal Azure account. They should be logged in at portal.azure.com (instead of labs.azure.com).
...
You will create a rule that blocks RDP on port `3389`.
...
- Source: This is the source computer. It can be a single IP, a range of addresses, an application security group, or a service tag.
    ...
    - For this rule, we will keep the source set to **Any**.
...
- Destination: We can choose between Any, IP address(s), VirtualNetwork, or an application security group. We'll choose **VirtualNetwork**, though we can also choose **Any**.
...
- Action: Firewall rules are either denying traffic or allowing traffic. Here, we are going to choose **Deny**.
...
- Name/Description: You can name a rule whatever you like, but it is a best practice to name it what it is and describe what it does. Here, we will name it "Block-RDP" and add the description: "Blocks RDP from all sources over port 3389."
...
In the last section of this class, you will deploy a computer onto our network.
...
- We will be prompted to save the SSH key into the default directory `/Users/cyber/.ssh/id_rsa`. Press the Enter key.

- We will be prompted to enter a password for our new SSH key.

- You **must** remember the password they set. They will not be able to recover it. If you forget the password, they will need to recreate their key.

- Our output should be similar to the below:
...
All SSH keys are stored in `~/.ssh`. We can see our private key named `id_rsa`, which should never be shared. We can also see a public key named `id_rsa.pub`, which we can place on servers that we want to access using this key.
...
- Copy the SSH key string and paste it into the Administrator Account section on the Basics page.

- The username can be any name. The SSH public key must be copied from the machine.
...
You should now have a VNet that holds their jump box protected by a firewall.

Even though we have added an SSH key to the VM, we currently cannot connect to it. This is intentional: the NSG that you implemented earlier prevents all traffic from reaching the machine. 
...
- We created the network and blocked all traffic before placing a VM inside of it. 
...
- Then, we added an SSH key through secure methods, ensuring that only the user with the correct private key (you) will be able to connect to the machine. 
    ...
    - This further protects the machine on the network by ensuring it can't be accessed, by the private key owner or any attacker who intercepted it, until the cloud administrator explicitly allows such access.
