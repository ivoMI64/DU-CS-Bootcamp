...
- This command does the following:  
   ...
   - Then, it opens that file and actively shows each line added, as the computer finds it.
...
- This command takes the output of the `ls` command and sends it into a file named `list.txt`. _If_ the file `list.txt` exists, it is overwritten with the output of the `ls` command.
...
- nFor example: `mkdir dir; cd dir; touch file; ls -l`
...
`mkdir dir && cd dir && touch file && ls -l`
   ...
   - If the command was written this way, `cd` will only run if `mkdir` was successful, `touch` only runs if `cd` was successful and `ls` only runs if `touch` was successful.
...
- Run `cd` to move to your home directory.
